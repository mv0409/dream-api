...
import { PagionationModel } from '../../helpers/interfaces/pagination.model';
	...
	return async (req: Request) : Promise<HttpResponse>  => {
		...
		try {
            ...
            if (result.dateBetween) {
                const date = new DateFormated(result.dateBetween.gt, result.dateBetween.lt)
                delete result.dateBetween
                Object.assign(result,date)
            }
            const totalResults = await Dream.find(result).countDocuments()

			const dream = await Dream.find(result)
                                     .skip(startIndex)
                                     .limit(limit);

            const pagination : PagionationModel = {}
